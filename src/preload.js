...
contextBridge.exposeInMainWorld('mods', {
  ...
  installed: () => ipcRenderer.invoke('sets:list'), activeSetId: () => ipcRenderer.invoke('set:active-id'), rollback: id => ipcRenderer.invoke('set:rollback', id), removeMod: payload => ipcRenderer.invoke('set:remove-mod', payload), purgeHistory: () => ipcRenderer.invoke('sets:purge'), chooseGameFolder: () => ipcRenderer.invoke('dialog:game-folder'), openFolder: folder => ipcRenderer.invoke('set:open-folder', folder), openSource: () => ipcRenderer.invoke('source:open'), openAppRepository: () => ipcRenderer.invoke('app-repository:open'),
