...
async function showHistory() {
  ...
  $('#historyClearGame').onclick = async () => { if (!await confirmStyled('Убрать активные моды из игры? Подготовленные наборы останутся в истории.', { title: 'Убрать из игры', okText: 'Убрать', danger: false })) return; try { const res = await window.mods.clearGame(); playChime('ok'); toast(res.removed ? `Активные моды убраны из игры (файлов: ${res.removed})` : 'В игре уже нет наших модов'); await refreshInstalled(); render(); showHistory(); } catch (error) { toast(error.message, true); } };
