...
// по своему разделу каталога, чтобы курьер не лежал в куче с эффектами.
  ...
  $('#cartList').innerHTML = state.cart.length ? [...groups.entries()].map(([hero, mods]) => `<section class="cart-group"><div class="group-title">${escapeHtml(hero).toUpperCase()}</div>${mods.map(mod => `<div class="cart-item"><div><div class="cart-item-name">${escapeHtml(mod.name)}</div><div class="cart-item-meta">${escapeHtml(mod.replaces)}</div></div><button class="remove" data-remove="${escapeHtml(mod.id)}" title="Удалить">×</button></div>`).join('')}</section>`).join('') : `<div class="cart-empty"><span>◌</span><p>Набор пока пуст</p><small>Добавьте моды из каталога. Они будут сгруппированы по героям.</small></div>`;
...
async function refreshCached() { await Promise.all(state.mods.map(async mod => { if (await window.mods.cached(mod.id)) state.cached.add(mod.id); })); }
...
$('#clearCart').onclick = async () => { if (!state.cart.length) return; if (!await confirmStyled('Очистить корзину? Набранные моды придётся добавлять заново.', { title: 'Очистить корзину', okText: 'Очистить', danger: false })) return; state.cart = []; render(); }; $('#applyButton').onclick = apply; $('#settingsButton').onclick = openSettings; $('#historyButton').onclick = showHistory; $('#refreshCatalog').onclick = refreshCatalog; $('#sourceButton').onclick = () => window.mods.openSource();
