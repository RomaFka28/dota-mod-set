...
async function removeInstalledMod(id) {
  ...
  try {
    ...
    render();
    playChime('ok');
    toast(`«${mod.name}» удалён из набора${result.wasInstalled ? ' и игры' : ''}. Остальные моды сохранены; скачанный файл оставлен в кэше.`);
  } catch (error) {
    playChime('err');
    if (button) { button.disabled = false; button.textContent = 'Убрать из набора'; }
