...
if (args.includes('--help') || args.includes('-h')) {
  console.log('Usage: node build-v2.js [--game <Dota game dir>] [--input <compiled vmdl>] [--output <pak02_dir.vpk>]');
  console.log('Defaults: DOTA_GAME_PATH or ./dota 2 beta; input/output use the expected addon and language paths.');
  process.exit(0);
}
...
const compiled = fs.readFileSync(option('--input', path.join(game, 'game', 'dota_addons', 'dota_mod_set_ws', 'models', 'merge', 'invoker_head.vmdl_c')));
const vpk = buildVpkV1([{ fullPath: 'models/heroes/invoker/invoker_head.vmdl_c', data: compiled }]);
const dest = option('--output', path.join(game, 'game', 'dota_russian', 'pak02_dir.vpk'));
